<!DOCTYPE html>
<html>
	<head>
		<meta charset="UTF-8">
		<meta name="viewport" content="width=device-width, initial-scale=1.0">
		<link rel = "stylesheet" href = "../css/about.css">
		<link rel="icon" type="image/png" href="../images/favicons/logo16x16.png" sizes="16x16">
		<link rel="icon" type="image/png" href="../images/favicons/logo32x32.png" sizes="32x32">
		<link rel="icon" type="image/png" href="../images/favicons/logo64x64.png" sizes="64x64">
		<link rel="icon" type="image/png" href="../images/favicons/logo128x128.png" sizes="128x128">
		<title>Despre noi</title>
	</head>
	<body>
		<div class = "header">
			<img src = "../images/about/logo.png" id = "logo_icon">
			<h1>TruckDeal</h1>
			<a id = "main_page" href = "main_page.php"></a>
		</div>
		<div class = "main-view">
			<div class = "backgr"></div>
			<div class = "content">
				<h1>Despre compania TruckDeal</h1>
				<div class = "text">
					<h2>1. Scurt istoric</h2>
					<p>&emsp;Compania a fost fondată în anul 2021, de către Mirel Popescu. El fiind în domeniul camioanelor de mulţi ani, îşi dorea mai mult de la propria sa viaţă profesională. Îşi dorea să revoluţioneze domeniul camioanelor cumva, dar nu ştia cum.</p>
					<p>&emsp;Iniţial, dorea să înfiinţeze o companie care oferă servicii de logistică. Pus pe ideea aceasta, se duce la diferiţi dealeri de camioane, unde constată că preţurile erau exagerat de mari. De asemenea, a căutat şi în străinate, unde erau, la fel, preţuri exagerate. Preţul prea mare la camioane era un impediment pentru un om de afaceri la început de drum.</p>
					<p>&emsp;Gândindu-se la toate cele întâmplate, dezamăgit, încearcă să găsească o soluţie pentru a-şi deschide propria companie, o dorinţă foarte mare pe care o avea. Într-un moment de inspiraţie, îşi dă seama că ar face un mare bine firmelor şi şoferilor pe cont propriu dacă şi-ar deschide o companie care să ofere posibilitatea cumpărarii camioanelor la preţuri rezonabile, cât mai simplu.  
					<p>&emsp;Încântat de idea pe care a avut-o, concepe un plan de afaceri pentru compania de vis din perspectiva sa. Cu acest plan de afaceri, merge la diferiţi producători de camioane, cum ar fi: Scania, Man, Renault, Daf, Mercedes-Benz s.a. Le prezintă planul de afaceri, iar la marea majoritate dintre firme, cei din conducere sunt foarte impresionaţi de ideile lui Mirel Popescu. E foarte fericit de cât de bine a mers, astfel încât, având susţinerea celor mai mari producători de camioane din lume, reuşeşte să îşi construiască compania mult visată.</p> 
					<p>&emsp;Nu a durat mult timp până când compania a devenit cunoscută, iar Mirel Popescu se bucură de succesul pe care îl are şi mai ales, de faptul că a reuşit sa ofere ajutor altor oameni la început de drum sau diferite companii să îşi marească flota de camioane, creând noi locuri de muncă.</p>
					<br><br>
					<h2>2. Viziunea companiei</h2>
					<p>&emsp;Compania TruckDeal îşi doreşte să ofere tuturor celor interesaţi de achiziţionarea de camioane servicii care să facă cumpărarea de camioane o încântare pentru fiecare client al nostru. Oferim o gamă largă de camioane din care puteţi alege pentru a vă dezvolta compania de vis.</p>
					<p>&emsp;Pentru a face lucrurile mai simple pentru clienţi, oferim transport gratuit pentru anumite camioane şi transport gratuit la achiziţionarea a două camioane, indiferent care ar fi ele. Pentru a rezerva camioane, pentru când vă veţi putea deplasa la reprezentanţa noastră, accesaţi magazinul nostru online -><a href = "store.php">aici</a><-. Camioanele rezervate vor fi ţinute pentru dumneavoastră cel mult 2 săptămâni, după care se va retrage statusul de rezervat şi vor putea fi rezervate de alţi clienţi.</p>
					<br><br>
					<h2>3. Reprezentanţă</h2>
					<p>&emsp;În prezent, compania TruckDeal are reprezentanţă în toate oraşele mari ale Romaniei. Pentru mai multe informaţii şi pentru a vedea adresa sediului principal, apăsaţi -><a href = "contact.php">aici</a><-.</p>
					<br><br>
					<h2>4. Capital</h2>
					<p>&emsp;În prezent, compania deţine un capital de €10.000.000, sedii în peste 15 oraşe şi peste 200 de angajaţi în total.</p>
					<br><br>
					<h2>5. Contact</h2>
					<p>&emsp;Pagina de contact o găsiţi -><a href = "contact.php">aici</a><-</p>
					<br><br>
					<h2>6. Parteneri</h2>
					<div class = "partners">
						<a href = "https://www.scania.com/"><img id = "scania" src = "../images/about/scania.png" loading = "eager" width = "400" height = "400"/></a>
						<a href = "https://www.man.com/"><img id = "man" src = "../images/about/man.png" loading = "eager" width = "1200" height = "672"/></a>
						<a href = "https://www.renault-trucks.com/en"><img id = "renault" src = "../images/about/renault.png" loading = "eager" width = "472" height = "800"/></a>
						<a href = "https://www.daf.com/en/sites-landing"><img id = "daf" src = "../images/about/daf.png" loading = "eager" width = "4620" height = "1530"/></a>
						<a href = "https://www.mercedes-benz-trucks.com/en_GB/home.php"><img id = "mercedes" src = "../images/about/mercedes-benz.png" loading = "eager" width = "500" height = "272"/></a>
						<a href = "https://www.iveco.com/uk/Pages/Home-Page.aspx"><img id = "iveco" src = "../images/about/iveco.png" loading = "eager" width = "800" height = "272"/></a>
						<a href = "https://www.volvo.com/"><img id = "volvo" src = "../images/about/volvo.png" loading = "eager" width = "420" height = "220"/></a>
					</div>
				</div>
			</div>
		</div>
		<?php
			if(!extension_loaded("gd"))
				echo '<script>
			console.warn("Librăria '."'gd'".' nu a fost încărcată. Dacă sunteţi administratorul site-ului asiguraţi-vă că aţi activat librăria '."'gd'".' în fişierul de configurare al php!");
		</script>
';
		?>
	</body>
</html>